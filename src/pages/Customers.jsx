    ...
    <div className="space-y-10">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div className="relative w-full md:w-[400px]">
          ...
          <input className="brutalist-input !pl-16 text-base py-4" placeholder="SEARCH CUSTOMERS..." value={search} onChange={(e) => setSearch(e.target.value)} />
        </div>
        <button className="brutalist-btn gap-4 w-full md:w-auto px-10 py-5 text-base bg-[var(--color-secondary)] text-[#ffffff]" onClick={openAddModal}>
          <Plus className="w-7 h-7" /> <span className="uppercase">ADD CUSTOMER</span>
        </button>
      </div>
                  ...
                  <tr key={c.id}>
                    <td>
                      <div>
                        ...
                        {stats.lastVisit && <span className="text-xs text-[var(--text-secondary)] font-bold uppercase">Last: {stats.lastVisit.toLocaleDateString()}</span>}
                      </div>
                    </td>
                    <td>
                    ...
                    <td>
                      {stats.totalDue === 0 ? (
                         ...
                         <span className="inline-flex items-center gap-1 text-xs font-black uppercase border-2 border-[var(--border-color)] bg-[var(--color-error)] text-[#ffffff] px-2 py-1 shadow-[2px_2px_0_var(--shadow-color)] animate-pulse">
                           <AlertCircle className="w-3 h-3" /> RISKY
                         </span>
                      ) : (
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title={editingCustomer ? "EDIT CUSTOMER" : "NEW CUSTOMER"}
        actions={
          <div className="flex gap-4">
            <button className="text-sm font-black uppercase underline decoration-2" onClick={() => setIsModalOpen(false)}>CANCEL</button>
      ...
      >
        <div className="space-y-6">
          <div>
            <label className="block text-sm font-black uppercase tracking-wider text-[var(--text-primary)] mb-2">Full Name *</label>
            <input className="brutalist-input text-base py-3" placeholder="John Doe" value={name} onChange={(e) => setName(e.target.value)} />
          </div>
          <div>
            <label className="block text-sm font-black uppercase tracking-wider text-[var(--text-primary)] mb-2">Phone Number *</label>
            <input className="brutalist-input text-base py-3" placeholder="9876543210" value={phone} onChange={(e) => setPhone(e.target.value)} />
          </div>
          <div>
            <label className="block text-sm font-black uppercase tracking-wider text-[var(--text-primary)] mb-2">Address</label>
          ...
          <div>
            <label className="block text-sm font-black uppercase tracking-wider text-[var(--text-primary)] mb-2">Notes</label>
            <input className="brutalist-input text-base py-3" placeholder="Any special instructions..." value={notes} onChange={(e) => setNotes(e.target.value)} />
          </div>
        </div>
      </Modal>
    </div>
